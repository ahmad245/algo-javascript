...
//File with id = 5 was changed at 461620206 and 461620207, so it should be included in the new backup as well. File with id = 6 was changed at 461620205, so it can be ignored.
...
function incrementalBackups(lastBackupTime, changes = []) {
    ...
    for (let i = 0; i < changes.length; i++) {
        if (changes[i][0] > lastBackupTime) {
            if (filesId.indexOf(changes[i][1]) == -1) {
                filesId.push(changes[i][1]);
            }
        }
    }
